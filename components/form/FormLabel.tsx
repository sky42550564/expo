...
export default ({
  ...
  labelColor = '#3d3d3d', // 便签的颜色
  needShowRequired = false, // 是否显示必选
}: any) => {
  const [state, setState] = useState({
    ...
    _needShowRequired: needShowRequired,
  });
  ...
  useEffect(() => {
    ...
    form.settings.needAlert != undefined && (state._needAlert = form.settings.needAlert); // 是否需要弹出错误框
    form.settings.labelColor != undefined && (state._labelColor = form.settings.labelColor); // 便签的颜色
    form.settings.needShowRequired != undefined && (state._needShowRequired = form.settings.needShowRequired); // 是否显示必选
    setState({ ...state });
    ...
    <Div s={[state._showFull ? `_fx_cc` : ``, state._showLine ? `_bob_f0f0f0` : `_wf`]}>
      <Div s={[`_ph_8 _hmin_46 _fx_r_ac`, topSep && `_mt_10`, bottomSep && `_mb_10`]}>
        ...
        <Div s='_fx_r'>
          ...
          {(state._needShowRequired && !noLabel && label && !disabled) && <Div s="_w_14 _c_red _fx_rc _pt_4">{required != false ? '*' : ''}</Div>}
          {
            (!noLabel && label && state._hasSpace) ?
              <Div s={['_nowrap _fx_rb', state._labelWidth && `_wm_${state._labelWidth}`]}>{_.map(label, (ch: any, i: any) => (<Text key={i} style={_u(`_fs_14_${state._labelColor}_400`)}>{ch}</Text>))}<Text style={_u(`_fs_14_${state._labelColor}_400`)}>{state._hasColon ? '：' : ''}</Text></Div>
              : (!noLabel && label) ?
                <Div s={[`_fs_${state._labelColor}_400 _nowrap _fx_r`, state._labelWidth && `_wm_${state._labelWidth}`]}>{`${label}${state._hasColon ? '：' : ''}`}</Div> : null
          }
        ...
        </Div>
        {
          ...
          (!state._showFull && !state._showRight) ?
            <Div s='_fx_r_1 _por'>
              {children}
              {unit && <Div s='_fs_12_red _self_ac _ml_6'>{ unit }</Div>}{/* 单位 */}
