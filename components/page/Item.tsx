...
export default (props: Props) => {
  ...
  const rows = useComputed(() => {
    ...
    list = _.filter(list, (o: any) => utils.visible(o.visible, { v: props.item[o.name], $: props.item, pageData: props.pageData, personal: personal.value, option: option.value })); // 过滤掉不显示的字段
    // 处理不断行的情况，如果一个filed
  ...
  const image = useComputed(() => { // 获取栏目头显示的图片的src
    ...
    return {
      ...
      style: field.list?.style || (field.value.type === 'head' ? _u(`_s_40`) : _u(`_s_60`)),
